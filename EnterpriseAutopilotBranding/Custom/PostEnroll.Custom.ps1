...
<#
.SYNOPSIS
Organization-owned extension point for post-enrollment work.

.DESCRIPTION
This script runs as SYSTEM after the first interactive sign-in. Keep it idempotent: check the
current state before changing anything, throw on failures that should cause a retry, and never
embed credentials or shared passwords.
