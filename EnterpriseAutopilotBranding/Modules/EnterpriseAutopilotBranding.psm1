...
function Import-EabConfiguration {
    ...
    if ([string]$config.SchemaVersion -ne '2') {
        throw "Unsupported configuration SchemaVersion '$($config.SchemaVersion)'. This release supports schema 2."
    }
    ...
    $enabledSteps = @($config.PostEnroll.Steps.Step | Where-Object {
        Get-EabBoolean -Value $_.Enabled -Default $true
    })
    if ($enabledSteps.Count -eq 0) {
        throw 'PostEnroll must contain at least one enabled <Step>.'
    }

    $stepIds = @{}
    $configurationDirectory = Split-Path -Path $resolvedConfigurationPath -Parent
    foreach ($step in @($config.PostEnroll.Steps.Step)) {
        $stepId = [string]$step.Id
        if ([string]::IsNullOrWhiteSpace($stepId) -or $stepId -notmatch '^[A-Za-z0-9][A-Za-z0-9._-]{1,79}$') {
            throw "PostEnroll step Id '$stepId' must contain 2-80 letters, numbers, periods, underscores, or hyphens."
        }
        if ($stepIds.ContainsKey($stepId)) {
            throw "PostEnroll step Id '$stepId' is duplicated."
        }
        $stepIds[$stepId] = $true

        try { $null = [version]$step.Version }
        catch { throw "PostEnroll step '$stepId' has invalid Version '$($step.Version)'." }

        $stepTimeout = [int]$step.TimeoutSeconds
        if ($stepTimeout -lt 5 -or $stepTimeout -gt 14400) {
            throw "PostEnroll step '$stepId' TimeoutSeconds must be between 5 and 14400."
        }

        $handler = [string]$step.Handler
        if ($handler -eq 'PowerShell') {
            if ([string]::IsNullOrWhiteSpace([string]$step.Path)) {
                throw "PowerShell onboarding step '$stepId' requires Path."
            }
            if (Get-EabBoolean -Value $step.Enabled -Default $true) {
                $stepScript = Resolve-EabChildPath -Path ([string]$step.Path) -BasePath $configurationDirectory
                if (-not (Test-Path -LiteralPath $stepScript -PathType Leaf)) {
                    throw "Enabled PowerShell onboarding step '$stepId' references missing script '$stepScript'."
                }
            }
        }
        elseif ($handler -eq 'WaitForFile' -and [string]::IsNullOrWhiteSpace([string]$step.DetectionPath)) {
            throw "WaitForFile onboarding step '$stepId' requires DetectionPath."
        }
        elseif ($handler -eq 'WaitForApplication' -and [string]::IsNullOrWhiteSpace([string]$step.DetectionNamePattern)) {
            throw "WaitForApplication onboarding step '$stepId' requires DetectionNamePattern."
        }
    }

    foreach ($excludedUser in @($config.PostEnroll.ExcludedUsers.User)) {
        if ([string]::IsNullOrWhiteSpace([string]$excludedUser.Pattern)) {
            throw 'PostEnroll excluded-user patterns cannot be empty.'
        }
    }
...
function Copy-EabRuntimeFiles {
    ...
    try {
        ...
        foreach ($directoryName in @('Modules', 'Custom', 'Onboarding')) {
...
function Register-EabPostEnrollTask {
    ...
    if (Get-EabBoolean -Value $settings.UiEnabled -Default $true) {
        $onboardingRoot = Join-Path $RuntimeRoot 'Onboarding'
        $launcherPath = Join-Path $onboardingRoot 'Invoke-AppDeployToolkit.exe'
        $launcherScript = Join-Path $onboardingRoot 'Invoke-AppDeployToolkit.ps1'
        $toolkitManifest = Join-Path $onboardingRoot 'PSAppDeployToolkit\PSAppDeployToolkit.psd1'
        foreach ($requiredPath in @($launcherPath, $launcherScript, $toolkitManifest)) {
            if (-not (Test-Path -LiteralPath $requiredPath -PathType Leaf)) {
                throw "PSADT onboarding runtime file '$requiredPath' was not staged."
            }
        }

        $action.Path = $launcherPath
        $action.Arguments = '-DeploymentType Install -DeployMode Interactive'
        $action.WorkingDirectory = $onboardingRoot
    }
    else {
        $action.Path = $powerShellPath
        $action.Arguments = "-NoProfile -NonInteractive -ExecutionPolicy Bypass -File `"$scriptPath`" -UiMode None"
