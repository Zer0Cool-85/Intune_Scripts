...
function Show-SystemSupportInfo {
    ...
    if ([System.Threading.Thread]::CurrentThread.ApartmentState -ne 'STA') {
        throw 'This WPF interface requires an STA PowerShell session. Run powershell.exe with the -STA option.'
    }
...
'@
    ...
    foreach ($field in @($Configuration.Fields)) {
        ...
        $copyButton.Add_Click({
            ...
            if (Set-SupportClipboard -Text $value) {
    ...
    $refreshData = {
        ...
        try {
            $state.Data = Get-SystemSupportData `
    ...
    $copySummary = {
        ...
        $summary = ConvertTo-SupportTicketSummary -Data $state.Data -Configuration $Configuration
        if (Set-SupportClipboard -Text $summary) {
