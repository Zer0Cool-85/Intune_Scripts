...
This package installs Cisco Secure Client Core, DART, and the Umbrella Roaming Security module as one Intune Win32 application. The wrapper is version-independent: it discovers the selected predeploy MSIs, verifies their Cisco signatures and MSI metadata, enforces matching versions, installs them in Cisco's required order, and writes a validated state file for Intune detection.
...
| --- | ---: | --- |
| `PackageRevision` | `2026.09.08.1` | Your revision for this exact Intune payload. Increment it for every repackaging and update the same value in the detection script. |
...
- Core, DART, and Umbrella are installed at the same version.
...
| Intune field | Value |
| --- | --- |
| Install command | `%SystemRoot%\SysNative\WindowsPowerShell\v1.0\powershell.exe -NoProfile -NonInteractive -ExecutionPolicy Bypass -File .\Install-CiscoSecureClient.ps1` |
| Uninstall command | `%SystemRoot%\SysNative\WindowsPowerShell\v1.0\powershell.exe -NoProfile -NonInteractive -ExecutionPolicy Bypass -File .\Uninstall-CiscoSecureClient.ps1` |
...
The installer uses MSI upgrade behavior; it does not preemptively uninstall the existing three modules. This avoids an unnecessary unprotected gap and lets MSI handle supported upgrades.
